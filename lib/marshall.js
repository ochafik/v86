...
marshall.Unmarshall = function(typelist, struct, state) {
    ...
    var outputOffset = 0;
    var output = new Array(typelist.length);
    for (var i=0; i < typelist.length; i++) {
        switch (typelist[i]) {
            case "w":
            ...
            case "h":
                var val = struct[offset++];
                output[outputOffset++] = (val + (struct[offset++] << 8));
                break;
            case "b":
                output[outputOffset++] = (struct[offset++]);
                break;
            case "s":
    ...
    dbg_assert(outputOffset == typelist.length, "Error in Unmarshall: didn't unpack expected number of values");
    state.offset = offset;
    return output;
};
